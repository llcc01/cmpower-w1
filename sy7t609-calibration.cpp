...
static process_state_t s_state;

void calibrationProcessHandler(void);

static uint32_t vrms_target;
static uint32_t irms_target;

static void printInfo(void);
static bool setControlRegisterAndVerify(void);
static bool setScaleAndVerify(void);
static bool setAccumAndVerify(void);
static void setAvgGain(void);
static bool isCalibrationComplete();
static bool startCalibration(uint32_t v_target, uint32_t i_target);
...
static bool isCalibrationComplete(void)
{
	uint32_t data;
...
static bool startCalibration(uint32_t mV, uint32_t mA)
{
	// emberAfAppPrintln("%s", __func__);
